...
function initializeTheme() {
  ...
  if (savedTheme) {
    ...
    const isDarkTheme = savedTheme === 'dark-theme' || (!savedTheme && window.matchMedia('(prefers-color-scheme: dark)').matches);
  ...
  if (themeToggleBtn) {
    themeToggleBtn.addEventListener('click', () => {
      if (document.body.classList.contains('dark-theme') || (!document.body.classList.contains('light-theme') && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
        document.body.classList.remove('dark-theme');
        document.body.classList.add('light-theme');
        localStorage.setItem(STORAGE_KEYS.THEME, 'light-theme');
        utils.changeThemeColor('#F8F9FA'); // Light theme color
      } else {
        document.body.classList.remove('light-theme');
        document.body.classList.add('dark-theme');
        localStorage.setItem(STORAGE_KEYS.THEME, 'dark-theme');
        utils.changeThemeColor('#121212'); // Dark theme color
      }
    });
  }
}
...
/**
 * Initialize panels (settings, history, more options)
 */
function initializePanels() {
  ...
  // Initialize global notification buttons
  initializeGlobalNotificationButtons();
}
